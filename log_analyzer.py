...
def parse_file_info(path):
    ...
    for line in fetch_file_lines(path):
        try:
            exp = re.search(
                r"\"((GET|POST|DELETE|PUT) (.+) HTTP/(1.0|1.1))(.+)(\d+\.\d{3})$", line
            )
            url = exp.group(3)
            ...
            stat_dict[url]["time_max"] = max(
                float(request_time), stat_dict[url]["time_max"]
            )
            stat_dict[url]["time_avg"] = (
                stat_dict[url]["time_avg"] * stat_dict[url]["count"]
                + float(request_time)
    ...
    for url in stat_dict.keys():
        stat_dict[url]["time_med"] = round(
            statistics.median(stat_dict[url]["time_values"]), 3
        )
        stat_dict[url]["count_perc"] = round(
            stat_dict[url]["count"] / requests_num * 100, 3
        )
        stat_dict[url]["time_perc"] = round(
            stat_dict[url]["time_sum"] / sum_request_time * 100, 3
        )
    ...
    try:
        if errors_num / requests_num >= ERROR_THRESHOLD:
            logger.info(
                f"Attention! {ERROR_THRESHOLD * 100} % of log file was not parsed!"
            )
...
def save_report(report_str, file_name, config):
	try:
		...
		logger.error(f"File {config['REPORT_DIR']} not found")
...
def prepare_dict_for_template(stat_dict, config):
    ...
    table_json_list = []
    stat_dict = dict(
        sorted(stat_dict.items(), key=lambda x: x[1]["time_sum"], reverse=True)[
            :report_size
        ]
    )
...
def parse_input_args(config_file):
    ...
    logging.basicConfig(
        filename=config.get("SCRIPT_LOGS_FILE"),
        level=logging.INFO,
...
def main():
    try:
        ...
        report_file_path = (
            f"report-{datetime.strftime(recent_log.date, '%Y.%m.%d')}.html"
        )
        save_report(report_str, report_file_path, config)

        logger.info(
            f"Report was successfully built and is stored in {report_file_path}"
        )
    except:
